...
class ProcessCount extends AbstractBaseChecker implements HealthCheckInterface
{
    ...
    /** @var string  */
    private $commandSignature = 'ps -ef | grep %s | wc -l';

    /** @var string  */
    private $message = 'Current running processes count of %s is %d, expected min %d and max %d';
    ...
    {
        $this->result = (int) $this->executeCommand($this->buildCommand()) -1;
    ...
    {
        return $this->result >= $this->getOption('min') && $this->result <= $this->getOption('max');
    }
